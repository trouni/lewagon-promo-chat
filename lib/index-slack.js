...
const refreshMessages = (forChannel) => {
  ...
  const url = `${baseUrl}/${forChannel}/messages`
  fetch(url)
    ...
    .then((data) => {
      ...
      // Prevent an older fetch to update messages after a channel change.
      if (forChannel === channel) {
        if (newMessages.length > 0) {
          ...
          if (messagesDiv.innerHTML === "Loading...") messagesDiv.innerHTML = 'No messages in this channel... yet.'
        }
      }
  });
};
...
const postMessage = (event) => {
  name = document.querySelector("#username").value;
  if (event.key === "Enter" && channel && name) {
    fetch(`${baseUrl}/${channel}/messages`, {
      method: "POST",
...
const initialize = () => {
  ...
  getChannels().forEach(chan => chan.addEventListener("click", toggleClassActive));
  document.addEventListener("DOMContentLoaded", () => setInterval(() => refreshMessages(channel), 1000));
  setMainChannel()
